...
namespace draw {
    ...
    inline SDL_Color get_pixel(const SDL_Surface* const surface, const uint x, const uint y)
    {
    ...
    inline void draw_flat_triangle_texture(const TexVertex &v0,  const TexVertex &v1,  const TexVertex &v2,
                                    const TexVertex &dv1, const TexVertex &dv2, TexVertex &e2, 
                                    const SDL_Surface *const texture, ZBuffer* zbuf)
    {
        TexVertex e1 = v0;
        ...
        for(int y=y_start; y < y_end; y++, e1 += dv1,e2 += dv2)
        {
            ...
            vec2d init_line = e1.texel + delta_line * (float(x_start) + 0.5 - e1.pos.x);
            for(int x=x_start; x<x_end; x++, init_line += delta_line)
            {
                // put pixel (x, y)
                // get pixel 
                auto b = std::fmod(init_line.y * tex_height, tex_clamp_y);
                auto pixel = get_pixel(texture, int(std::fmod(init_line.x * tex_width, tex_clamp_x)), int(std::fmod(init_line.y * tex_height, tex_clamp_y)));
                auto z = util::vec::interpolate_z(global_t->pts, vec2d (x, y));
                zbuf->put_pixel(vec3d(x, y, z), pixel);
            }
        }
    }
    inline void draw_flat_top_triangle_texture(const TexVertex& v0, const TexVertex& v1, const TexVertex& v2, const SDL_Surface* const texture, ZBuffer* zbuf) 
    {
    ...
    inline void draw_flat_bottom_triangle_texture(const TexVertex& v0, const TexVertex& v1, const TexVertex& v2, const SDL_Surface* const texture, ZBuffer* zbuf) 
    {
        const float delta_y = v2.pos.y - v0.pos.y;
    ...
    inline void draw_triangle_texture(triangle pos_tri, triangle tex_tri, const SDL_Surface* const texture, ZBuffer* zbuf) 
    {
        global_t = &pos_tri;
        ...
        { 
            ...
            const TexVertex v3 (util::vec::interpolate(v0.pos, v2.pos, split), util::vec::interpolate(v0.texel, v2.texel, split));
